...
function lerProdutos($conexao){
   /*  $sql = "SELECT id, nome, preco, quantidade, descricao, fabricante_id
    ...
    produtos.preco, produtos.descricao, fabricantes.nome AS fabricante FROM produtos INNER JOIN fabricantes
    ON produtos.fabricante_id = fabricantes.id ORDER BY produto";

    $resultado = mysqli_query($conexao, $sql) or(mysqli_error($conexao));
    $produtos = [];
